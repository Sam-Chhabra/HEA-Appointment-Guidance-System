...
export class GuidanceService {
    submitGuidanceInput(inputText, patientId) {
        // Validate input
        ...
        const session = guidanceRepository.create(patientId, inputText, JSON.stringify(keywords), recommendation.departmentId, recommendation.departmentId // Initially, selected = recommended
        );
        return {
            isEmergency: false,
    ...
    overrideDepartment(sessionId, departmentId, patientId) {
    ...
    getGuidanceSession(sessionId, patientId) {
        const session = guidanceRepository.findById(sessionId);
        if (!session) {
            throw new NotFoundError('Guidance session not found.');
        }

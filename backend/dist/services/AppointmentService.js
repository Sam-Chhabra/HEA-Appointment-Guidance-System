...
import { isValidTransition } from '../utils/validation.js';
...
export class AppointmentService {
     ...
     */
    bookAppointment(patientId, doctorId, timeSlotId, departmentId, reasonOrNeed, patientInfo) {
        const db = getDb();
    ...
    `).run(patientInfo.fullName, patientInfo.dateOfBirth, patientInfo.phoneNumber, patientId);
        ...
        const bookTransaction = db.transaction(() => {
            ...
            const appointment = appointmentRepository.create(patientId, doctorId, timeSlotId, departmentId, reasonOrNeed, 'CONFIRMED');
            // Mark slot as BOOKED
            timeSlotRepository.updateStatus(timeSlotId, 'BOOKED');
            // Audit log
            auditLogRepository.create(patientId, 'APPOINTMENT_BOOKED', 'appointment', appointment.id, JSON.stringify({ doctorId, timeSlotId, departmentId }));
            return appointment;
        });
        ...
        try {
            ...
            notificationRepository.create(patientId, 'CONFIRMATION', `Your appointment with Dr. ${apptDetails?.doctor_name} on ${apptDetails?.slot_start_time} has been confirmed.`, appointment.id);
     ...
     */
    modifyAppointment(appointmentId, newTimeSlotId, actorUserId) {
        const db = getDb();
        ...
        const rescheduleTransaction = db.transaction(() => {
            ...
            // Audit log
            auditLogRepository.create(actorUserId, 'APPOINTMENT_RESCHEDULED', 'appointment', appointmentId, JSON.stringify({ oldSlotId: appointment.time_slot_id, newSlotId: newTimeSlotId }));
        });
        ...
        try {
            ...
            notificationRepository.create(actorUserId, 'UPDATE', `Your appointment has been rescheduled to ${apptDetails?.slot_start_time}.`, appointmentId);
     ...
     */
    cancelAppointment(appointmentId, actorUserId) {
        const db = getDb();
        const appointment = appointmentRepository.findById(appointmentId);
        if (!appointment)
            throw new NotFoundError('Appointment not found.');
        ...
        const cancelTransaction = db.transaction(() => {
            ...
            timeSlotRepository.updateStatus(appointment.time_slot_id, 'AVAILABLE');
            auditLogRepository.create(actorUserId, 'APPOINTMENT_CANCELLED', 'appointment', appointmentId, JSON.stringify({ timeSlotId: appointment.time_slot_id }));
        });
        cancelTransaction();
        // === END TRANSACTION ===
        // Notification
        try {
            notificationRepository.create(actorUserId, 'CANCELLATION', `Your appointment has been cancelled.`, appointmentId);
    ...
    viewPastAppointments(patientId) {
        return appointmentRepository.findPastByPatient(patientId);
    }
    viewFutureAppointments(patientId) {

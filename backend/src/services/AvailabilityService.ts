...
export class AvailabilityService {
  ...
  addAvailability(adminId: number, doctorId: number, startTime: string, endTime: string) {
    ...
    const addTransaction = db.transaction(() => {
      ...
      auditLogRepository.create(
        ...
        'time_slot',
        slot.id,
        JSON.stringify({ doctorId, startTime, endTime })
      );
  ...
  removeAvailability(adminId: number, slotId: number) {
    ...
    const removeTransaction = db.transaction(() => {
      ...
      auditLogRepository.create(
        ...
        'time_slot',
        slotId,
        JSON.stringify({ doctorId: slot.doctor_id, startTime: slot.start_time, endTime: slot.end_time })
      );
    });
  ...
  getDoctorAvailability(doctorId: number, from?: string, to?: string) {
    const doctor = doctorRepository.findById(doctorId);
    if (!doctor) throw new NotFoundError('Doctor not found.');
